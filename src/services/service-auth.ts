...
const authenticateCode = async (code_user: string) => {
  try {
    ...
    if (response.ok && data.success) {
      // Si l'authentification est réussie, stocke l'userId et le code_user dans le store
      useAuthStore.getState().setUserId(data.userId)
      useAuthStore.getState().setCodeUser(code_user) // Stocke également le code utilisateur
